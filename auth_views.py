...
# Scoped rate throttles for auth endpoints
...
def get_auth_token(request):
    ...
    try:
        ...
        if user.check_password(password):
            # Create a Django session for the user (sets HttpOnly session cookie)
            django_login(request, user)
            # Ensure CSRF cookie/token is present for subsequent requests
            csrf_token = get_csrf_token(request)
            logger.warning(f"Successful login (session created): {email}")
            resp = Response({
                'user_id': user.id,
                'email': user.email,
                'name': user.name,
            ...
            # Set CSRF cookie explicitly (not HttpOnly) so frontend JS can read it if needed
            ...
            logger.warning(f"Failed login attempt: {email} - Invalid password")
...
def register_user(request):
    ...
    if serializer.is_valid():
        ...
        # Log the user in using Django sessions (creates HttpOnly session cookie)
        django_login(request, user)
        csrf_token = get_csrf_token(request)
        resp = Response({
            'user_id': user.id,
